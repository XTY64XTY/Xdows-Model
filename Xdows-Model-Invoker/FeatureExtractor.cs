...
public static class ProHybridFeatureExtractor
{
    ...
    public static ProHybridFileFeatures ExtractFromBytes(byte[] bytes)
    {
        ...
        GetHeadTailBytes(bytes, headLen, tailStart, out ReadOnlySpan<byte> headBytes, out ReadOnlySpan<byte> tailBytes);
    ...
    private sealed class RegionStats
    {
        ...
        public void ComputeDerivedStats(int length)
        {
            if (length <= 0) return;

            Span<long> counts = Counts;
            ByteAnalysisHelper.ComputeStatsSummary(counts, length,
                out int uniqueBytes, out double mostCommonByteRatio, out double zeroByteRatio);
            ByteAnalysisHelper.ComputeByteMoments(counts, length,
                out double mean, out double variance, out double skewness, out double kurtosis);
            ByteAnalysisHelper.ComputeByteRangeRatios(counts, length,
                out double low, out double printable, out double extended);

            UniqueBytes = uniqueBytes;
            MostCommonByteRatio = mostCommonByteRatio;
            ZeroByteRatio = zeroByteRatio;
            Entropy = ByteAnalysisHelper.ComputeEntropy(counts, length);
            MeanByteValue = mean;
            ByteValueVariance = variance;
            ByteDistributionSkewness = skewness;
            ByteDistributionKurtosis = kurtosis;
            LowByteRatio = low;
            PrintableAsciiRatio = printable;
            ExtendedAsciiRatio = extended;
        }
    }
        ...
        out RegionStats rawTailStats)
    {
        ...
        fullStats.ComputeDerivedStats(len);
        headStats.ComputeDerivedStats(headLen);
        tailStats.ComputeDerivedStats(len - tailStart);
        rawHeadStats.ComputeDerivedStats(rawHeadLen);
        rawMidStats.ComputeDerivedStats(rawMidEnd - rawMidStart);
        rawTailStats.ComputeDerivedStats(len - rawTailStart);
    }

    private static void GetHeadTailBytes(byte[] bytes, int headLen, int tailStart, out ReadOnlySpan<byte> headBytes, out ReadOnlySpan<byte> tailBytes)
    {
        ...
        headBytes = bytes.AsSpan(0, FlashFeatureExtractor.FlashRegionSize);
        tailBytes = bytes.AsSpan(tailStart, FlashFeatureExtractor.FlashRegionSize);
    }
    ...
    private static FileFeatures BuildStandardFeatures(byte[] bytes, RegionStats stats)
    {
        ...
        features.UniqueBytes = stats.UniqueBytes;
        features.MostCommonByte = mostCommonByte;
        features.MostCommonByteRatio = stats.MostCommonByteRatio;
        features.LeastCommonByte = leastCommonByte;
        features.LeastCommonByteRatio = (double)minCount / length;
        features.ZeroByteRatio = stats.ZeroByteRatio;
        features.HighEntropyRatio = (double)stats.HighByteCount / length;
        features.Entropy = stats.Entropy;
        ...
        features.MeanByteValue = stats.MeanByteValue;
        features.ByteValueVariance = stats.ByteValueVariance;
        features.ByteDistributionSkewness = stats.ByteDistributionSkewness;
        features.ByteDistributionKurtosis = stats.ByteDistributionKurtosis;
        ...
        features.LowByteRatio = stats.LowByteRatio;
        features.PrintableAsciiRatio = stats.PrintableAsciiRatio;
        features.ExtendedAsciiRatio = stats.ExtendedAsciiRatio;
    ...
    private static FlashFileFeatures BuildFlashFeatures(ReadOnlySpan<byte> headBytes, ReadOnlySpan<byte> tailBytes, RegionStats headStats, RegionStats tailStats, long fileSize)
    {
        ...
        features.UniqueBytes = headStats.UniqueBytes;
        features.MostCommonByteRatio = headStats.MostCommonByteRatio;
        features.ZeroByteRatio = headStats.ZeroByteRatio;
        features.HighEntropyRatio = (double)headStats.HighByteCount / headLen;
        features.Entropy = headStats.Entropy;
        ...
        features.MeanByteValue = headStats.MeanByteValue;
        features.ByteValueVariance = headStats.ByteValueVariance;
        features.ByteDistributionSkewness = headStats.ByteDistributionSkewness;
        features.ByteDistributionKurtosis = headStats.ByteDistributionKurtosis;
        ...
        features.LowByteRatio = headStats.LowByteRatio;
        features.PrintableAsciiRatio = headStats.PrintableAsciiRatio;
        features.ExtendedAsciiRatio = headStats.ExtendedAsciiRatio;
        ...
        if (tailBytes.Length > 0 && !tailBytes.SequenceEqual(headBytes))
        {
    ...
    private static void FillSectionFromStats(RegionStats stats, int fileSize, float[] destination, int offset)
    {
        ...
        destination[offset + 0] = (float)stats.Entropy;
        ByteAnalysisHelper.ComputeByteHistogram32(byteCounts, length, destination.AsSpan(offset + 1, 32));

        destination[offset + 33] = (float)stats.PrintableCount / length;
        destination[offset + 34] = (float)byteCounts[0] / length;
